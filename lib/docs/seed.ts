  ...
  "data": [
    {
      ...
      "Банк": "ПАО «Банк DEMO»",
      "НомерСчета": "40702810900000012345",
      "ВидСчета": "Расчетный",
      "Валюта": "руб.",
      "Закрыт": false,
...
| \`data[].НомерСчета\` | string | \`БанковскиеСчета.НомерСчета\` | Номер банковского счёта. Для кассы пусто |
...
Добавлен метод **Кассовые документы** — \`GET /main/hs/cashorders/get\` (\`GetCashOrders\`). Приходные и расходные кассовые ордера одной выборкой.

Добавлен метод **Банковские документы** — \`GET /main/hs/bankpayments/get\` (\`GetBankPayments\`). Платёжные поручения (входящие и исходящие) и платёжные ордера поступления и списания.

Оба метода отдают шапку документа, а при \`full=true\` — все реквизиты шапки и все табличные части построчно, парами ключ-значение. Состав ключей читается из метаданных документа и поэтому различается между типами: перед чтением конкретного ключа проверяйте его наличие.

Параметр \`type\` в обоих методах применяется при построении запроса: опрашиваются только подходящие типы документов. Если не подошёл ни один, \`data\` пустой, а причина в \`errors\`.

**cashflows/get:** в ответ добавлено поле \`НомерСчета\` (номер банковского счёта; у кассы пусто) и фильтр \`accountNo\` по подстроке в нём.
...
В отличие от [ведомости по денежным средствам](/api/1c-upp/methods/cashflows-get), которая отдаёт **итоги по регистру**, этот метод отдаёт **сами документы** - вплоть до всех строк их табличных частей.
...
### Отбор в запросе к базе

| Параметр | Тип | Описание |
|----------|-----|----------|
| \`from\` | string | Начало периода. Действует **только вместе с** \`to\` |
| \`to\` | string | Конец периода, включительно до конца дня |
| \`date\` | string | Один день. Применяется **дополнительно** к \`from\`/\`to\`, а не вместо них |
| \`code\` | string | Подстрока в номере документа (\`ПОДОБНО %code%\`) |
| \`summ\` | number | Точное совпадение по \`СуммаДокумента\` |
| \`approved\` | boolean | Фильтр по признаку проведения |
| \`type\` | string | Опрашивать только типы, чьё имя содержит подстроку, например \`ПриходныйКассовыйОрдер\`. Применяется **при построении запроса**, а не после него. Если не подошёл ни один тип, \`data\` пустой, причина в \`errors\` |
...
|----------|-----|-------------|----------|
| \`org\` | string | \`Организация\` | Подстрока в наименовании организации |
| \`contractor\` | string | \`Контрагент\` | Подстрока в контрагенте. Синоним: \`contactor\` |
| \`account\` | string | \`СчетКасса\` | Подстрока в кассе или банковском счёте организации |
| \`responsible\` | string | \`Ответственный\` | Подстрока в ответственном |

### Прочие

| Параметр | Тип | Описание |
|----------|-----|----------|
| \`full\` | boolean | Полный ответ: все реквизиты шапки и все табличные части. Ключ читается как \`full\`, \`Full\` и \`FULL\` |
...
GET /main/hs/cashorders/get?contractor=Поставщик&from=01.02.2025&to=28.02.2025
GET /main/hs/cashorders/get?account=Основная&responsible=Сидоров
GET /main/hs/cashorders/get?code=00-000123&full=true
\`\`\`

## Формат ответа

Без \`full\` возвращается шапка и сводка по табличным частям:
  ...
  "data": [
    {
      ...
      "СчетКасса": "Основная касса",
      "ВидОперации": "Оплата от покупателя",
      "Валюта": "руб.",
      "Ответственный": "Сидоров С.С.",
      ...
      "ТабличныеЧасти": { "Есть": true, "Количество": 1 }
    }
  ],
  "errors": []
}
...
С \`full=true\` вместо сводки приходят все реквизиты шапки и все строки ТЧ:
...
Шапка приходит всегда:

| Поле | Тип | Описание |
|------|-----|----------|
| \`data[].ТипДокумента\` | string | Имя объекта метаданных, например \`ПриходныйКассовыйОрдер\` |
| \`data[].Номер\` | string | Номер документа |
| \`data[].ДатаДокумента\` | string | Дата (\`ДД.ММ.ГГГГ\`) |
| \`data[].Дата\` | string | То же значение, для совместимости с остальными методами |
| \`data[].Организация\` | string | Представление организации, либо \`Не указана\` |
| \`data[].Контрагент\` | string | Представление контрагента, либо \`Не указан\` |
| \`data[].СчетКасса\` | string | Касса или банковский счёт организации, либо \`Не указан\` |
| \`data[].ВидОперации\` | string | Вид операции документа, либо пустая строка |
| \`data[].Валюта\` | string | Валюта документа, либо пустая строка |
| \`data[].Ответственный\` | string | Ответственный, либо \`Не указан\` |
| \`data[].СуммаДокумента\` | number | Сумма документа, \`0\` если не заполнена |
| \`data[].Комментарий\` | string | Комментарий, либо пустая строка |
| \`data[].Проведен\` | boolean | Признак проведения |
| \`data[].ПометкаУдаления\` | boolean | Признак пометки на удаление |

Дальше состав зависит от \`full\`:

| Поле | Режим | Описание |
|------|-------|----------|
| \`data[].ТабличныеЧасти\` | без \`full\` | Сводка \`{ "Есть": boolean, "Количество": number }\` - суммарное число строк по всем ТЧ |
| \`data[].Реквизиты\` | \`full=true\` | Объект: ключ - имя реквизита шапки из метаданных, значение - приведённое значение |
| \`data[].ТабличныеЧасти\` | \`full=true\` | Объект: ключ - имя ТЧ, значение - массив строк; в каждой строке \`НомерСтроки\` и пара ключ-значение на каждый реквизит ТЧ |

> Состав \`Реквизиты\` и \`ТабличныеЧасти\` **не фиксирован**. Он читается из метаданных конкретного документа, поэтому у разных типов набор ключей разный. Не полагайтесь на наличие конкретного ключа - проверяйте его перед чтением.
...
| Дата | строка \`ДД.ММ.ГГГГ\` |
| Ссылка на объект | строковое представление |
| Пустая ссылка, \`NULL\`, \`Неопределено\` | пустая строка \`""\` |

## Ошибки и производительность

- Если объект документа не удалось прочитать, строка **всё равно** попадает в \`data\` - с пустыми \`Реквизиты\` / \`ТабличныеЧасти\`, а причина уходит в \`errors\`. HTTP-код остаётся **200**.
- Каждый документ читается через \`ПолучитьОбъект()\`, то есть одно обращение к базе на документ. На широком периоде без фильтров ответ будет тяжёлым - сужайте период либо отбирайте по \`type\`.
- Сортировка сквозная по всем типам: дата по убыванию, затем номер по убыванию.
`,
};
...
| \`summ\` | number | Точное совпадение по \`СуммаДокумента\` |
| \`approved\` | boolean | Фильтр по признаку проведения |
| \`type\` | string | Опрашивать только типы, чьё имя содержит подстроку, например \`ПлатежноеПоручениеВходящее\`. Применяется **при построении запроса**, а не после него. Если не подошёл ни один тип, \`data\` пустой, причина в \`errors\` |
...
GET /main/hs/bankpayments/get?contractor=Поставщик&from=01.02.2025&to=28.02.2025
GET /main/hs/bankpayments/get?account=Основная&responsible=Сидоров
...
| Поле | Тип | Описание |
|------|-----|----------|
| \`data[].ТипДокумента\` | string | Имя объекта метаданных, например \`ПлатежноеПоручениеВходящее\` |
| \`data[].Номер\` | string | Номер документа |
| \`data[].ДатаДокумента\` | string | Дата (\`ДД.ММ.ГГГГ\`) |
| \`data[].Дата\` | string | То же значение, для совместимости с остальными методами |
| \`data[].Организация\` | string | Представление организации, либо \`Не указана\` |
