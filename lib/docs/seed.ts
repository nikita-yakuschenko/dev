...
export const stockBalancesGetDoc: DocInput = {
  title: "Получить стоимость остатков товаров",
  slug: "methods/stockbalances-get",
  section: "1c-upp",
  navTitle: "Стоимость остатков",
  order: 604,
  content: `# Получить стоимость остатков товаров

HTTP-сервис \`stockbalances\`, метод \`GetStockBalances\`. Возвращает количество и стоимость остатков номенклатуры на складах по регистру накопления **ПартииТоваровНаСкладах** (\`РегистрНакопления.ПартииТоваровНаСкладах.Остатки\`) на указанную дату среза.
...
| \`name\` | string | Фильтр по наименованию номенклатуры. Поиск **ПОДОБНО**. Символ \`/\` передавайте как \`_slash_\` |
...
GET /main/hs/stockbalances/get?date=2025-03-15&warehouse=Объект
  ...
  "data": [
    {
      ...
      "НоменклатурнаяГруппа": "Стройматериалы",
      "ДатаСреза": "15.03.2025",
      "ИтогоКоличество": 165.5,
      "ИтогоСтоимость": 496500,
      "СредняяЦена": 3000,
      "Остатки": [
        {
          "Склад": "Склад «Северный»",
...
В регистре остатки читаются из виртуальной таблицы \`ПартииТоваровНаСкладах.Остатки\`. При \`full=true\` базовый список строится по справочнику \`Номенклатура\`, поэтому позиции без строк остатков попадут в \`data\` с пустым массивом \`Остатки\`.
...
| \`data[].ИтогоКоличество\` | number | сумма \`КоличествоОстаток\` | Суммарный остаток по всем складам в позиции |
| \`data[].ИтогоСтоимость\` | number | сумма \`СтоимостьОстаток\` | Суммарная стоимость остатка по всем складам в позиции |
| \`data[].СредняяЦена\` | number | расчет | \`ИтогоСтоимость / ИтогоКоличество\`, 4 знака |
| \`data[].Остатки\` | array | \`ПартииТоваровНаСкладах.Остатки\` | Остатки по складам |
| \`data[].Остатки[].Склад\` | string | \`Склад\` | Наименование склада |
| \`data[].Остатки[].Количество\` | number | \`КоличествоОстаток\` | Количество на складе |
| \`data[].Остатки[].Стоимость\` | number | \`СтоимостьОстаток\` | Стоимость остатка на складе |
| \`data[].Остатки[].СредняяЦена\` | number | расчет | \`Стоимость / Количество\`, 4 знака |
...
- Без \`code\` и \`name\` возвращается вся номенклатура с остатками (с учётом \`full\` и фильтра склада).
...
| Стоимость остатков | \`GET /main/hs/stockbalances/get\` |
